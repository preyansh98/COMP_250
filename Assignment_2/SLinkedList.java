...
public class SLinkedList<E extends DeepClone<E>> implements Iterable<E>
{
	...
	/*
 	...
 	{
 		SLinkedList<E> clonedlinkedlist = new SLinkedList<E>();
 		SNode<E> tmp = head;
			 		if(head == null) {
			 				return clonedlinkedlist;
			 			}
			 		
 		clonedlinkedlist.head = new SNode<E>(head.element.deepClone());
 		SNode<E> nexttemp = clonedlinkedlist.head;
 		clonedlinkedlist.size++;
 		
			 		while(!(tmp.equals(tail))) {
			 			tmp=tmp.next;
			 			nexttemp.next = new SNode<E>(tmp.element.deepClone());
			 			nexttemp= nexttemp.next;
			 			clonedlinkedlist.size++; 
			 		}

 		clonedlinkedlist.tail=nexttemp;
 		return clonedlinkedlist; 		
